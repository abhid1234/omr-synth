...
from src.synth.render import JianpuRenderer, VerovioRenderer
...
def build_demo(output: Path = Path("demo/index.html")) -> Path:
    ...
    renderers = (VerovioRenderer(), JianpuRenderer())
    cards: list[str] = []
...
<article class="sample">
  <div class="sample-head"><h2>Sample {index:02d}</h2><div class="tags"><span>curriculum level {level}</span><span>Western</span><span>Jianpu</span><span>seed {seed}</span></div></div>
  <p class="mode">Same symbolic record · two render grammars{degradation}</p>
  <div class="pair">
